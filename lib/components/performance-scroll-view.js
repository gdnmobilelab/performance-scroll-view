...
import { ItemBuffer } from "./item-buffer";
...
export class PerformanceScrollView extends Component {
    ...
    get isAtScrollBottom() {
        ...
        return (Math.abs(this.state.totalHeight - this.state.container.clientHeight - this.state.currentScrollPosition) <=
            1);
    }
    renderChildItems() {
        if (!this.state.container) {
        ...
        return this.state.itemBuffer.map((child, idx) => {
            ...
            return (React.createElement(ScrollViewItem, { debugId: indexInFullItemList.toString(), key: keyPrefix + "_" + indexInFullItemList, itemIndex: indexInFullItemList, onRender: this.itemRendered, y: yPosition }, child));
        });
    }
    renderMoreIndicator() {
        ...
        return (React.createElement("div", { style: containerStyles, onClick: this.scrollToEnd }, moreIndicator));
    }
    render() {
        return (React.createElement("div", { id: this.props.id, style: this.state.mergedContainerStyles, className: this.props.className, ref: this.setContainer },
            this.renderChildItems(),
            React.createElement(DummyScroller, { ref: el => (this.dummyScrollContainer = el), height: this.state.totalHeight, onScroll: this.containerScrolled, onIdle: this.onIdle, scrollPosition: this.state.currentScrollPosition }),
